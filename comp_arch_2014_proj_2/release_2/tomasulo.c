...
int execute(mathOp mathOpType, executeRequest_t *executeRequest) {
	...
	if(mathOpType == add) {
		for(i = 0; i < add_rs_num; i++) {
			if((!add_rs[i].is_available) &&((add_rs[i].op1_tag) == -1) && ((add_rs[i].op2_tag) == -1) &&(!add_rs[i].is_being_executed)) {
		...
		for(i = 0; i < mul_rs_num; i++) {
			if((!mul_rs[i].is_available) &&((mul_rs[i].op1_tag) == -1) && ((mul_rs[i].op2_tag) == -1)&&(!mul_rs[i].is_being_executed)) {
...
void place_instruction(reservation_station rs, register_file rf, instruction_t *theInstruction) {
	int op1, op2;
	if (theInstruction->instructionType == addImm || theInstruction->instructionType == multImm) {
		op2 = theInstruction -> op2;
	}
