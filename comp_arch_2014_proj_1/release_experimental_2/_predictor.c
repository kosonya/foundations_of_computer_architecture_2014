...
int getLocalPrediction(const branch_record_t *br) {
  bool isBranchTaken = false;
  if (predictor_data.local_prediction_table[predictor_data.local_history_table[(br->instruction_addr)%1024].value].value >= 8) isBranchTaken = true;
  return (int)isBranchTaken;
}
...
//update local prediction table
  if (predictor_data.local_prediction_table[predictor_data.local_history_table[(br->instruction_addr)%1024].value].value < 15 && is_taken) {
