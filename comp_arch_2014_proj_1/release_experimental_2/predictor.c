...
void initPredictor() {
	int i;
	for(i = 0; i < 1024; i++) {
		predictor_data.local_history_table[i].value = 0;
		predictor_data.local_history_table[i].pc = 0;
		predictor_data.local_history_table[i].is_used = false;
	}
/*	FILE *f;
...
uint getPCIndex(const branch_record_t *br) {
	int i;
	for(i = 0; i < 1024; i++) {
		if (predictor_data.local_history_table[i].is_used && predictor_data.local_history_table[i].pc == br->instruction_addr) {
			return i;
		}
	}
	for(i = 0; i < 1024; i++) {
		if (!predictor_data.local_history_table[i].is_used) {
			predictor_data.local_history_table[i].is_used = true;
			predictor_data.local_history_table[i].pc = br->instruction_addr;
		}
	}
	return 0;
