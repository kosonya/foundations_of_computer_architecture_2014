...
uint getMSB(const branch_record_t *br) {
	return (((br -> instruction_addr) >> (32-12)) ^ predictor_data.global_history_register.value)%4096;
}
...
int getGlobalPrediction(const branch_record_t *br) {
  ...
  if (predictor_data.global_prediction_table[getMSB(br)].value >= 2) isBranchTaken = true;
...
int getPrediction(branch_record_t *br) {
  ...
  else if (predictor_data.chooser_prediction_table[getMSB(br)].value >= 2) {
...
//update global prediction table
  if (predictor_data.global_prediction_table[getMSB(br)].value < 3 && is_taken) {
    predictor_data.global_prediction_table[getMSB(br)].value++;
  } else if (predictor_data.global_prediction_table[getMSB(br)].value > 0 && (!is_taken)) {
        predictor_data.global_prediction_table[getMSB(br)].value--;
  }
...
//update chooser prediction table
  ...
  if (is_taken == local_prediction && predictor_data.chooser_prediction_table[getMSB(br)].value > 0)
    predictor_data.chooser_prediction_table[getMSB(br)].value--;
  if (is_taken == global_prediction && predictor_data.chooser_prediction_table[getMSB(br)].value < 3)
    predictor_data.chooser_prediction_table[getMSB(br)].value++;
